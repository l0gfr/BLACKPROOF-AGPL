...
assert.equal(status.securityPosture.noDocumentUpload, true);
assert.equal(status.posture.noDocumentUpload, true);
...
const forbidden = /\/api\/license|startCheckout|refreshLicense|Licence activée|Créer et exporter nécessite|\bStripe\b|\babonnement\b|offre Solo|FAQ clients?/i;
for (const path of walk(root).filter((file) => /\.(html|js|json)$/.test(file))) {
  ...
  if (path.endsWith(".html") && content.includes("Content-Security-Policy")) {
    ...
    assert.equal((content.match(/data-share-panel/g) ?? []).length, 1, `One shared privacy-safe toolbar is required in ${path}`);
    for (const destination of ["x", "facebook", "linkedin", "email"]) {
      assert.ok(content.includes(`data-share-destination="${destination}"`), `Missing ${destination} share link in ${path}`);
    }
  }
}
